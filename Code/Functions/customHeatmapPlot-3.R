...
# ){
  ...
  simprofResults <- simprof(data = mat-1, method.cluster = 'ward.D2'
  ...
  # Clolourblind friendly pallette
  pal <- c("#3216ba" # Indigo
           , "#17a446" # Green
           , "#e05785" # Rose
           , "#4776a7" # Teal
  ...
  pal <- pal[1:length(unique(Clusters$Cluster))]
  names(pal) <- unique(Clusters$Cluster)
  Clusters$Colour <- pal[as.character(Clusters$Cluster)]
  ...
  d_horiz <- Chi_dist(mat_horiz-1)
  dend_horiz <- as.dendrogram(hclust(d_horiz, method = "ward.D2"))
  dend_data_horiz <- dendro_data(dend_horiz)
  dend_data_horiz 
  
  simprofResults_horiz <- simprof(data = mat_horiz-1, method.cluster = 'ward.D2'
                            , method.distance = Chi_dist)
  ...
  Phylum_pos_table <- merge(Phylum_pos_table, Clusters, by = "Phylum")
 ...
 # }))
  ...
  plt_hmap <- ggplot(heatmap_data, 
                     aes(x = factor(x_center_true), y = y_center
                         , fill = Abundance, 
...
customHeatmapPlot <- cmpfun(customHeatmapPlot)
