...
ggcorrplot <- function(corr, method = c("circle", "square", "ellipse", "number"),
                       ...
                       pch = 4, pch.cex = 5
                       , colourscale.direction = "vertical"
                       , y.label.angle = 0
                       , y.label.hjust = "center") {
    ...
    if (method %in% c("ellipse", "circle", "square")) {
      p <- p + scale_fill_gradientn(colours = col2(200), limits = c(-1, 1),
                                    guide = guide_colorbar(
                                      ...
                                      ticks.colour = "black",
                                      frame.colour = "black",
                                      barwidth = ifelse(colourscale.direction == "vertical"
                                                        , yes = 1.5, no = 15),
                                      barheight = ifelse(colourscale.direction == "vertical"
                                                         , yes = 15, no = 1.5)))
    } else {
      p <- p + scale_colour_gradientn(colours = col2(200), limits = c(-1, 1),
                                      guide = guide_colorbar(
                                        title = "",
                                        direction = colourscale.direction,
                                        nbin = 1000,
                                        ticks.colour = "black",
                                        frame.colour = "black",
                                        barwidth = ifelse(colourscale.direction == "vertical"
